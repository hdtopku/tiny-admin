<template>
  <div>
    ...
    >
      <template #title
      >{{ $t('移除秒杀商品 - ') }}{{ flashInfo.activityName }}
      </template>
      <template #footer>
      ...
      <div class="mb-4">
        <a-tag
        >{{ $t('原有秒杀商品数：') }}{{ flashInfo?.goodsIds?.length }}
        </a-tag
        >
        -
        <a-tag
            v-if="flashInfo.goodsIds.length - selectedRowKeys.length === 0"
            color="red"
            @close.prevent="popConfirmVisible = true"
        >{{
            $t('当前移除商品数：')
          }}{{ flashInfo.goodsIds.length - selectedRowKeys.length }}
        </a-tag>
        <a-popconfirm
            v-else
        ...
        =
        <a-tag color="blue"
        >{{ $t('剩余关联商品数：') }}{{ selectedRowKeys.length }}
        </a-tag
        >
      </div>
      <a-input :placeholder="$t('请输入商品名称、商品id、描述')" allow-clear>
        <template #suffix>
...
const handleOk = () => {
  ...
  removeGoods(flashInfo.value.id, [...originalGoodsIdSet])
      .then(() => {
