...
import {getSmsHotGoodsPage, saveOrUpdateSmsHotGoods} from '@/api/sms/hotGoods.ts'
...
import {deleteSmsNewGoods} from "@/api/sms/newGoods.ts";
...
const openModal = (record: any) => {
  modalRef.value.openModal(record, saveOrUpdateSmsHotGoods)
}
...
const changeRecordStatus = (record: any) => {
  record.loading = true
  saveOrUpdateSmsHotGoods({id: record.id, status: !record.status}).then(() => {
