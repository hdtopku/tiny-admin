...
import {saveOrUpdateNewGoods} from "@/api/sms/newGoods.ts";
...
const handleOk = () => {
  ...
  saveOrUpdateNewGoods(newGoodsList).then(() => {
...
<template>
  <div>
    <a-modal v-model:open="open" :title="isUpdate? '编辑新品' : '新增新品'" cancel-text="取消" destroy-on-close
             ok-text="提交"
             @ok="handleOk">
      <template #footer>
